...
class BlogActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        testRef.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                Log.d("TEST_BLOG", "Snapshot exists: ${snapshot.exists()}")
                Log.d("TEST_BLOG", "Children count: ${snapshot.childrenCount}")
                for (child in snapshot.children) {
                    Log.d("TEST_BLOG", "Key: ${child.key}, content: ${child.child("content").value}")
                }
            }

            override fun onCancelled(error: DatabaseError) {
                Log.e("TEST_BLOG", "Cancelled: ${error.message}")
            }
        })
    ...
    private fun showBottomSheetDialog() {
        ...
        bottomSheetBinding.btnPostBlog.setOnClickListener {
            ...
            val title = bottomSheetBinding.title.text.toString().trim()
            if (content.isEmpty()) {
    ...
    private fun fetchBlogPosts() {
        Log.d("BLOG_FETCH", "Feed mode: $currentFeedMode")

        blogRef.limitToLast(50)
            .addListenerForSingleValueEvent(object : ValueEventListener {
                ...
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    when (currentFeedMode) {
                        FeedMode.NEW -> {
                            // Newest first
                            blogList.sortByDescending { it.timestamp }
                        }
